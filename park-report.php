...
<body>
	...
	<div style="float: right;color: black;"><?php $currentDateTime=date('d-m-y H:i');?>
...
$todate=$_POST['todate'];
?>
	<h4>Park Report from: <?php echo htmlentities($fromdate);?> to date: <?php echo htmlentities($todate);?></h4>
...
$sql = "SELECT tblusers.FullName,tblusers.EmailId,tblusers.Idno,tblusers.ContactNo,tblbrands.ParkName,tblbrands.Location,tblbrands.PricePerDay,tblbrands.lots,tblbrands.booked, tblbooking.BookingNumber,tblbooking.carno,tblbooking.FromDate,tblbooking.ToDate,tblbooking.status, tblbooking.pid from tblbooking join tblbrands on tblbrands.id=tblbooking.pid join tblusers on tblusers.EmailId=tblbooking.EmailId where tblbooking.pid=:pid && tblbrands.id=:pid && tblbooking.bookingdate BETWEEN '$FromDate' AND '$todate'" ;
//AND postinga date range $from date to $todate
...
<td><?php echo htmlentities($result->PricePerDay);?></td>
					...
					<?php $cnt=$cnt+1; }} ?>
											...
											<form method="post">
	   <input name="Submit2" type="submit" class="txtbox4" id="print" value="Print" onClick="printDiv()" style="cursor: pointer;"  />
	</form>
